use ../rss/_shared.nu *
# CNN top stories via RSS feed. Returns a table with title, link, published, summary, author.
@category news
export def "prim-cnn-feed" [
    --limit: string = "20"   # Max items to return
]: nothing -> table {
    let doc = (http get --raw -H {User-Agent: "Mozilla/5.0 (compatible; junction-box-news/1.0)"} "http://rss.cnn.com/rss/edition.rss" | from xml)
