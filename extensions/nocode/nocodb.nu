...
export def "prim-nocodb-query" [
    ...
    # If NOCODB_HOST is a full URL (e.g. https://app.nocodb.com/base), extract parts.
    let url = if ($h | str starts-with "http://") or ($h | str starts-with "https://") {
        ...
        # NocoDB v2 API uses absolute path: /api/v2/tables/{id}/records
        # When NOCODB_HOST includes a base prefix, use full path.
        let full_path = $"/api/v2/tables/($tbl)/records"
        ({
            scheme: $scheme,
            host: $host_part,
            path: $"($base_path)($full_path)",
            params: $params
        } | url join)
    } else {
        ({
