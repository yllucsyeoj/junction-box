# HTTP POST — send data to a URL. --url and --headers are wirable. Use headers for Authorization: {"Authorization": "Bearer sk-..."}.
@category external
export def "prim-http-post" [
    --url: string = ""             # [wirable][required] URL to POST to
    ...
    if ($h | is-empty) {
        $body_json | http post --content-type $content_type $url_val
    } else {
        $body_json | http post --content-type $content_type --headers $h $url_val
    }
}
