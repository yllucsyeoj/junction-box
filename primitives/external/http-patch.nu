# HTTP PATCH — update a resource. --url and --headers are wirable. Use headers for Authorization: {"Authorization": "Bearer sk-..."}.
@category external
export def "prim-http-patch" [
    --url: string = ""               # [wirable][required] URL to PATCH
    ...
    if ($h | is-empty) {
        $body_json | http patch --content-type $content_type $url_val
    } else {
        $body_json | http patch --content-type $content_type --headers $h $url_val
    }
}
